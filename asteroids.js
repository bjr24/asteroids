...
(function() {
  var Asteroid, Bullet, BulletBill, Coin, Drawable, FireBall, FireFlower, HealthBar, ImageLoader, Mushroom, PowerUp, QuestionBlock, Ship, Star, StarPowerUp, Vec2, asteroids, bindControls, bulletBills, bullets, canvas, canvasHeight, canvasWidth, coins, draw, drawBackground, fpsToInterval, frameRate, gameInit, healthBar, hits, images, incrementHitCount, incrementScore, numCoins, numFireFlowersPickedUp, onLoad, powerUps, questionBlocks, score, ship, stars, thrust, updateCoefficient, xformCanvas,
    ...
    images = new ImageLoader([
      {
        ...
        name: "green-shell",
        url: imgBaseUrl + "img/green-shell.png"
      }, {
        name: "coin",
        url: imgBaseUrl + "img/coin.png"
      }, {
        name: "bulletbill",
        url: imgBaseUrl + "img/bulletbill.png"
      }, {
        name: "question-block",
        url: imgBaseUrl + "img/question-block.png"
      }
    ]);
  ...
  draw = function() {
    var a, b, c, p, qb, _i, _j, _k, _l, _len, _len1, _len2, _len3, _len4, _len5, _m, _n;
    drawBackground();
    ship.draw();
    ...
    for (_l = 0, _len3 = coins.length; _l < _len3; _l++) {
      c = coins[_l];
      c.draw();
    }
    bulletBills = bulletBills.filter(function(b) {
      return b.exists();
    });
    for (_m = 0, _len4 = bulletBills.length; _m < _len4; _m++) {
      b = bulletBills[_m];
      b.draw();
    }
    for (_n = 0, _len5 = questionBlocks.length; _n < _len5; _n++) {
      qb = questionBlocks[_n];
      qb.draw();
    }
  ...
  incrementScore = function(n) {
    if (n == null) {
      n = 1;
    }
    score += n;
    return $("#scoreDisplay").text(score);
  };

  incrementHitCount = function(n) {
    if (n == null) {
      n = 1;
    }
    hits += n;
    return $("#hitsDisplay").text(hits);
  };
  ...
  Ship = (function(_super) {
    ...
    function Ship() {
      ...
      this.checkCoinPickup = __bind(this.checkCoinPickup, this);
      this.checkPowerUpPickup = __bind(this.checkPowerUpPickup, this);
      this.checkInvinCollision = __bind(this.checkInvinCollision, this);
    ...
    Ship.prototype.update = function() {
      ...
      if (this.isInvin) {
        this.checkInvinCollision(asteroids);
        this.checkInvinCollision(bulletBills);
      } else {
        this.checkCollision(asteroids);
        this.checkCollision(bulletBills);
      }
      this.checkPowerUpPickup();
      return this.checkCoinPickup();
    };

    Ship.prototype.checkCollision = function(list) {
      if (list.some((function(_this) {
        return function(e) {
          return _this.closeEnough(e, e.size.x);
        };
      })(this))) {
        healthBar.decrement();
    ...
    Ship.prototype.checkInvinCollision = function(list) {
      var e, _i, _len, _results;
      _results = [];
      for (_i = 0, _len = list.length; _i < _len; _i++) {
        e = list[_i];
        if (this.closeEnough(e, e.size.x)) {
          _results.push(e.onHit(this));
        }
      }
      return _results;
    };
    ...
    Ship.prototype.checkCoinPickup = function() {
  ...
  Asteroid = (function(_super) {
    ...
    function Asteroid(type, parent, hitter, first) {
      ...
      this.image = images.get("green-shell");
      this.size = new Vec2(this.image.width, this.image.height).scaleToWidth(sizeTable[this.type]);
      this.force = new Vec2(0, 0);
      if (parent != null) {
        ...
        if (Math.randInt(2) === 0) {
          Asteroid.__super__.constructor.call(this, 0, null);
        } else {
          Asteroid.__super__.constructor.call(this, null, 0);
        }
    ...
    Asteroid.prototype.update = function() {
      ...
      for (_i = 0, _len = bullets.length; _i < _len; _i++) {
        b = bullets[_i];
        if (!(this.closeEnough(b, this.size.x))) {
          continue;
        }
        b.onHit();
        hitter = b;
  ...
  PowerUp = (function(_super) {
    ...
    PowerUp.prototype.lifetimeExceeded = function() {
      return (new Date().getTime() - this.createTimeMs) > 30000;
    };
  ...
  StarPowerUp = (function(_super) {
    ...
    function StarPowerUp(x, y) {
      StarPowerUp.__super__.constructor.call(this, "star", x, y);
      this.size = this.size.scaleToWidth(38);
    }
  ...
  Coin = (function(_super) {
    ...
    function Coin(parent) {
      this.exists = __bind(this.exists, this);
      this.onPickup = __bind(this.onPickup, this);
      this.render = __bind(this.render, this);
      Coin.__super__.constructor.call(this, parent.position.x, parent.position.y);
      this.image = images.get("coin");
      this.size = new Vec2(this.image.width, this.image.height).scaleToWidth(25);
      this.pickedUp = false;
    }

    Coin.prototype.render = function(c) {
      return this.drawImage(c, this.image);
    };

    Coin.prototype.onPickup = function() {
      this.pickedUp = true;
      numCoins += 1;
    ...
    Coin.prototype.exists = function() {
  ...
  BulletBill = (function(_super) {
    __extends(BulletBill, _super);
    ...
    BulletBill.prototype.render = function(c) {
      return this.drawImage(c, this.image);
    };

    BulletBill.prototype.update = function() {
      var b, hitThisTime, _i, _len;
      BulletBill.__super__.update.apply(this, arguments);
      hitThisTime = false;
      for (_i = 0, _len = bullets.length; _i < _len; _i++) {
        b = bullets[_i];
        if (!(this.closeEnough(b, this.size.x - 20))) {
          continue;
        }
        hitThisTime = true;
        b.onHit();
    ...
    BulletBill.prototype.exists = function() {
      return !this.gotHit && this.position.x < canvasWidth;
    };

    BulletBill.prototype.onHit = function() {
      this.gotHit = true;
      incrementScore(30);
      incrementHitCount();
      return coins.push(new Coin(this));
  ...
  QuestionBlock = (function(_super) {
    ...
    function QuestionBlock(x, y) {
      ...
      this.update = __bind(this.update, this);
      this.render = __bind(this.render, this);
      QuestionBlock.__super__.constructor.call(this, x, y);
      this.image = images.get("question-block");
      this.size = new Vec2(this.image.width, this.image.height).scaleToWidth(35);
    }

    QuestionBlock.prototype.render = function(c) {
      return this.drawImage(c, this.image);
    };

    QuestionBlock.prototype.update = function() {
      var b, hitThisTime, _i, _len;
      QuestionBlock.__super__.update.apply(this, arguments);
      hitThisTime = false;
      for (_i = 0, _len = bullets.length; _i < _len; _i++) {
        b = bullets[_i];
        if (!(this.closeEnough(b, this.size.x))) {
    ...
    QuestionBlock.prototype.onHit = function() {
      var powerUp;
      powerUp = (function() {
        switch (Math.randInt(3)) {
          case 0:
            return new Mushroom(this.position.x, this.position.y);
          case 1:
            return new FireFlower(this.position.x, this.position.y);
          case 2:
            return new StarPowerUp(this.position.x, this.position.y);
        }
      }).call(this);
      powerUps.push(powerUp);
      this.position.x = Math.randInt(canvasWidth);
      return this.position.y = Math.randInt(canvasHeight);
    };
